...
import {KitchenSinkData} from '../../Constant/Constant';
import {Checkbox, RadioButton} from 'react-native-paper';

const KitchenSinkFilter = () => {
  const [selectedCategory, setSelectedCategory] = useState('Categories');
  const [selectedKitchenSink, setSelectedKitchenSink] = useState([]);
  ...
  const toggleKitchenSink = categories => {
    setSelectedKitchenSink(prev =>
          ...
          <View style={{flex: 1}}>
            <ScrollView
              ...
              {selectedCategory === 'Categories' && (
                <FlatList
                  data={KitchenSinkData.categories}
                  keyExtractor={item => item.id.toString()}
                  renderItem={({item}) => (
                    <TouchableOpacity
                      onPress={() => toggleKitchenSink(item.categories)}
                      style={{
                        flexDirection: 'row',
                        alignItems: 'center',
                        // padding: 5,
                      ...
                      <Checkbox
                        status={
                          selectedKitchenSink.includes(item.categories)
                            ? 'checked'
                            : 'unchecked'
                        }
                        onPress={() => toggleKitchenSink(item.categories)}
              ...
              {selectedCategory === 'Sale Type' && (
                <FlatList
                  data={KitchenSinkData.saleTypes}
                  keyExtractor={item => item.id.toString()}
                  renderItem={({item}) => (
                    <TouchableOpacity
              ...
              {selectedCategory === 'Price Type' && (
                <FlatList
                  data={KitchenSinkData.priceTypes}
                  keyExtractor={item => item.id.toString()}
                  renderItem={({item}) => (
                    <TouchableOpacity
              ...
              {selectedCategory === 'Verified User' && (
                <RadioButton.Group
                  ...
                  <FlatList
                    data={KitchenSinkData.VerifiedUser}
                    keyExtractor={item => item.id.toString()}
                    renderItem={({item}) => (
                      <TouchableOpacity
...
export default KitchenSinkFilter;
